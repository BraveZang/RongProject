...
- (UIColor *)titleColor {
    if (!_titleColor) {
        _titleColor = [MTool colorWithHexString:@"333333"];
    }
    return _titleColor;
}
...
- (void)loadAddressData{
    ...
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES);
     ...
     self.areAry= [[NSArray alloc] initWithContentsOfFile:filename];
    
    
//
//
//
//    NSString * filePath = [[NSBundle mainBundle] pathForResource:@"c"
//                                                          ofType:@"txt"];
//    NSError  * error;
...
//    _pArr         = [[NSMutableArray alloc]init];
    ...
    _provincesArr = [NSMutableArray arrayWithArray:self.areAry];
    //省份模型数组加载各个省份模型
