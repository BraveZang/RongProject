...
- (void)reloadDataWithModel:(OKLogisticModel*)model {
    ...
    if (self.currented) {
        [dsc addAttribute:NSForegroundColorAttributeName value:[MTool colorWithHexString:@"#FF5448"] range:NSMakeRange(0, model.dsc.length)];
    }else {
...
- (void)setCurrented:(BOOL)currented {
    ...
    if (currented) {
        self.infoLabel.textColor = [MTool colorWithHexString:@"#FF5448"];
    } else {
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.currented) {
        ...
        UIColor *cColor = OKRGBAColor(255, 84, 72, 1.0);
        [cColor set];
        [cicle fill];
        
        UIColor *shadowColor = OKRGBAColor(255, 84, 72, 0.5);
        [shadowColor set];
